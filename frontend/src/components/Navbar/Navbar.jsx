...
const Navbar = () => {
  ...
  return (
    <div className='flex bg-zinc-800 text-white px-6 py-4 items-center justify-between'>
      ...
      <div>
        <Link to='/' className='text-2xl font-semibold'>Bookish</Link>
      </div>

      <div className='hidden md:flex items-center gap-4'>
        {links.map((item, i) => (
          <Link className='hover:text-blue-500 transition-all duration-300' to={item.link} key={i}>
            {item.title}
          </Link>
        ))}
      </div>
        ...
        <Link to={'/sign-in'} className='px-2 py-1 border border-blue-500 rounded hover:bg-white hover:text-zinc-800 transition-all duration-300'>
          LogIn
        </Link>
        <Link to={'/sign-up'} className='px-2 py-1 bg-blue-500 rounded hover:bg-white hover:text-zinc-800 transition-all duration-300'>
          SignUp
        </Link>
      </div>
          )
